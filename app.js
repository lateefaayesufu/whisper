...
const App = (() => {
  ...
  function setupAuthUI() {
    ...
    $('register-form').addEventListener('keydown', e => { if (e.key === 'Enter') handleRegister(); });

    // Login
    $('login-btn').addEventListener('click', handleLogin);
    $('login-form').addEventListener('keydown', e => { if (e.key === 'Enter') handleLogin(); });
  }
  ...
  async function handleRegister() {
    ...
    if (!username || !displayName || !password) return showAuthError('All fields are required.', 'register');
    if (password !== confirm) return showAuthError('Passwords do not match.', 'register');
    if (password.length < 8) return showAuthError('Password must be at least 8 characters.', 'register');
    ...
    try {
      ...
      const { wrappedPrivateKey, pbkdf2Salt } = await Crypto.wrapPrivateKey(keyPair.privateKey, password);
  ...
  async function handleLogin() {
    ...
    if (!username || !password) return showAuthError('Username and password required.', 'login');
  ...
  function setAuthLoading(on, label = 'Loading…', form) {
    ...
    if (loadEl) { loadEl.textContent = on ? label : ''; loadEl.style.display = on ? 'block' : 'none'; }
  }
  ...
  async function openConversation(user) {
    ...
    const convItem = document.querySelector(`.conv-item[data-userid="${user.id}"]`);
    ...
    $('chat-avatar').textContent = getInitials(user.display_name || user.username);
  ...
  async function loadMessageHistory(userId) {
    ...
    try {
      ...
      if (!messages || messages.length === 0) {
        container.innerHTML = '<div class="msg-empty">No messages yet. Say hello!</div>';
        return;
      }
  ...
  async function appendMessage(msg, scroll = true) {
    ...
    // Decrypt async
    try {
      const plaintext = await Crypto.decryptPayload(msg.payload, privateKey, isSender);
  ...
  async function handleSendMessage() {
    ...
    try {
      const payload = await Crypto.encryptForRecipient(text, recipientPubKey, selfPubKey);
  ...
  function connectWebSocket() {
    WS.on('connected', () => {
      ...
      $('ws-status').className = 'ws-status online';
    });

    WS.on('disconnected', () => {
      $('ws-status').textContent = '🟡 Reconnecting…';
      $('ws-status').className = 'ws-status offline';
    });
    ...
    WS.on('message.receive', async (msg) => {
      ...
      if (conv && (msg.from_user_id === conv.id || msg.to_user_id === conv.id)) {
      ...
      if (!conv || msg.from_user_id !== conv.id) {
        showToast(`New message from ${msg.from_user_id}`, 'info');
      }
    });
    ...
    WS.on('user.online', ({ user_id }) => {
      ...
      if (dot) { dot.className = 'conv-presence online'; dot.title = 'Online'; }
    });
    ...
    WS.on('user.offline', ({ user_id }) => {
      ...
      updateOnlineIndicator(user_id);
      const dot = document.getElementById(`presence-${user_id}`);
      if (dot) { dot.className = 'conv-presence'; dot.title = ''; }
    });
